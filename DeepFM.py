...
class DeepFM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            # FM model
            # tf.nn.embedding_lookup()的作用，是选取一个张量里面索引对应的元素
            self.embeddings = tf.nn.embedding_lookup(self.weights['feature_embeddings'],self.feat_index) # N * F * K
            feat_value = tf.reshape(self.feat_value,shape=[-1,self.field_size,1])
            self.embeddings = tf.multiply(self.embeddings,feat_value) # 元素级别相乘
    ...
    def _initialize_weights(self):
        ...
        #embeddings
        weights['feature_embeddings'] = tf.Variable(tf.random_normal([self.feature_size,self.embedding_size],0.0,0.01),name='feature_embeddings') # 是一个矩阵
